...
from XD_Viajes.models import XD_Viajes, XD_PedidosxViajes, XD_Pedidos, XD_AccesoriosxViajes, XD_EvidenciasxPedido, \
    XD_EvidenciasxViaje, Ext_Viajes_MesaControl as Ext_Viajes_MesaControlXD, ServiciosXD
...
from bkg_viajes.models import Bro_Viajes, Bro_EvidenciasxViaje, Ext_Viajes_MesaControl, Servicios
...
def EvidenciasProveedor(request):
    if request.user.roles == 'MesaControl' or request.user.is_superuser:
        EvidenciasxAprobar = XD_Viajes.objects.filter(Status='FINALIZADO',
                                                      FechaDespacho__month=datetime.datetime.now().month,
                                                      FechaDespacho__year=datetime.datetime.now().year).exclude(
            Status='CANCELADO')
        EvidenciasxAprobarBKG = Bro_Viajes.objects.filter(StatusProceso='FINALIZADO',
                                                          FechaDescarga__month=datetime.datetime.now().month,
                                                          FechaDescarga__year=datetime.datetime.now().year).exclude(
            StatusProceso='CANCELADO')
        EvidenciasxAprobarServicios = ServiciosXD.objects.filter(StatusProceso='FINALIZADO')
        Evidencias = chain(EvidenciasxAprobar, EvidenciasxAprobarBKG, EvidenciasxAprobarServicios)
        ...
        return render(request, 'EvidenciasProveedor.html',
                      {'Evidencias': EvidenciasByWendo, 'EvidenciasxAprobar': Evidencias,
                       'EvidenciaDigital': SinEvidenciaDigital, 'Proveedores': Proveedores})
...
def FindFolioProveedorE(request):
    ...
    try:
        ...
        if 'XDD' in Folio:
            IsServicio = True if 'XDDMNB' in Folio else False
            XDFolio = XD_Viajes.objects.exclude(Status='CANCELADO').get(Folio=Folio, Status='FINALIZADO',
                                                                        IDTransportista=request.user.IDTransportista) if not IsServicio else ServiciosXD.objects.get(
                Folio=Folio, StatusProceso='FINALIZADO', IDTransportista=request.user.IDTransportista)
            if IsServicio:
                GetEvidenciasxServicio = XD_EvidenciasxViaje.objects.filter(IDServicio=XDFolio.IDServicio)
                if XD_EvidenciasxViaje.objects.filter(IDServicio=XDFolio.IDServicio).exists():
                    for each in GetEvidenciasxServicio:
                        newDelivery = {}
                        newDelivery['XD_IDPedido'] = each.IDEvidenciaxViaje
                        newDelivery['Delivery'] = XDFolio.Folio
                        newDelivery['IDViaje'] = XDFolio.IDServicio
                        newDelivery['TipoEvidencia'] = 'Servicio'
                        newDelivery['RutaArchivo'] = ""
                        newDelivery['Status'] = 'Rechazada' if (
                                    each.IsEnviada and each.IsRechazada) else 'Aprobada' if (
                            each.IsValidada) else "Enviada" if (
                                    each.IsEnviada and not each.IsRechazada and not each.IsValidada) else "Otro"
                        newDelivery['ComentarioRechazo'] = each.ComentarioRechazo
                        arrFoliosEvidencias.append(newDelivery)
                else:
                    newDelivery = {}
                    newDelivery['XD_IDPedido'] = XDFolio.IDServicio
                    newDelivery['Delivery'] = XDFolio.Folio
                    newDelivery['IDViaje'] = XDFolio.IDServicio
                    newDelivery['TipoEvidencia'] = 'Servicio'
                    newDelivery['RutaArchivo'] = ""
            ...
            elif XDFolio.TipoViaje == 'CUSTODIA':
                GetEvidenciaxManiobra = XD_EvidenciasxViaje.objects.filter(IDXD_Viaje=XDFolio.XD_IDViaje,
                                                                           Tipo='EVCUSTODIAF', Titulo="FOLIO")
                if GetEvidenciaxManiobra:
                    for DataEvidenciaxManiobra in GetEvidenciaxManiobra:
                        ...
                        newDelivery['RutaArchivo'] = '' if (
                                    DataEvidenciaxManiobra.IsEnviada and DataEvidenciaxManiobra.IsRechazada) else DataEvidenciaxManiobra.RutaArchivo
                        newDelivery['Status'] = 'Rechazada' if (
                                    DataEvidenciaxManiobra.IsEnviada and DataEvidenciaxManiobra.IsRechazada) else 'Aprobada' if (
                            DataEvidenciaxManiobra.IsValidada) else "Enviada" if (
                                    DataEvidenciaxManiobra.IsEnviada and not DataEvidenciaxManiobra.IsRechazada and not DataEvidenciaxManiobra.IsValidada) else "Otro"
                ...
                GetDelivery = XD_PedidosxViajes.objects.filter(
                    XD_IDViaje=XDFolio.XD_IDViaje)  # , StatusPedido = 'ENTREGADO')
                if GetDelivery:
                    ...
                    if len(EvidenciaByObservacion) >= 1:
                        for eachpedido in EvidenciaByObservacion:
                            if not XD_EvidenciasxPedido.objects.filter(XD_IDViaje=eachpedido["IDViaje"],
                                                                       IDXD_Pedido=eachpedido["XD_IDPedido"],
                                                                       Titulo='BITACORA').exists():
                                ...
                                newDelivery['RutaArchivo'] = ""
                                newDelivery['Status'] = "Enviada" if XD_EvidenciasxPedido.objects.filter(
                                    XD_IDViaje=eachpedido["IDViaje"], IDXD_Pedido=eachpedido["XD_IDPedido"],
                                    Titulo='BITACORA', IsValidada=0, IsRechazada=0,
                                    IsEnviada=1).exists() else "Aprobada" if XD_EvidenciasxPedido.objects.filter(
                                    XD_IDViaje=eachpedido["IDViaje"], IDXD_Pedido=eachpedido["XD_IDPedido"],
                                    Titulo='BITACORA', IsValidada=1, IsRechazada=0,
                                    IsEnviada=1).exists() else "Rechazada" if XD_EvidenciasxPedido.objects.filter(
                                    XD_IDViaje=eachpedido["IDViaje"], IDXD_Pedido=eachpedido["XD_IDPedido"],
                                    Titulo='BITACORA', IsValidada=0, IsRechazada=1, IsEnviada=1).exists() else "Otro"
                                newDelivery['ComentarioRechazo'] = ""
                                arrFoliosEvidencias.append(newDelivery)
                    for Delivery in GetDelivery:
                        if len(XD_EvidenciasxPedido.objects.filter(IDXD_Pedido=Delivery.XD_IDPedido.XD_IDPedido,
                                                                   XD_IDViaje=Delivery.XD_IDViaje.XD_IDViaje,
                                                                   Titulo='EVIDENCIA1')) >= 1:
                            TieneEvidencia = XD_EvidenciasxPedido.objects.filter(
                                IDXD_Pedido=Delivery.XD_IDPedido.XD_IDPedido, XD_IDViaje=Delivery.XD_IDViaje.XD_IDViaje,
                                Titulo='EVIDENCIA1')
                            for TieneEvidencia1 in TieneEvidencia:
                                newDelivery = {}
                                newDelivery['XD_IDPedido'] = Delivery.XD_IDPedido.XD_IDPedido
                                newDelivery['Delivery'] = Delivery.XD_IDPedido.Delivery.translate({ord(c): "" for c in
                                                                                                   "!@#$%^&*()[]{};:,./<>?\|`~-=_+"}) if TieneEvidencia1.Titulo != 'BITACORA' else GetObservacionesByPedidoT1(
                                    Delivery.XD_IDPedido.Observaciones, Delivery.TipoTransporte)
                                newDelivery['IDViaje'] = Delivery.XD_IDViaje.XD_IDViaje
                                newDelivery[
                                    'TipoEvidencia'] = 'Pedido' if TieneEvidencia1.Titulo != 'BITACORA' else 'Bitacora'
                                newDelivery['RutaArchivo'] = '' if (
                                            TieneEvidencia1.IsEnviada and TieneEvidencia1.IsRechazada) else TieneEvidencia1.RutaArchivo
                                newDelivery['Status'] = 'Rechazada' if (
                                            TieneEvidencia1.IsEnviada and TieneEvidencia1.IsRechazada) else 'Aprobada' if (
                                    TieneEvidencia1.IsValidada) else "Enviada" if (
                                            TieneEvidencia1.IsEnviada and not TieneEvidencia1.IsRechazada and not TieneEvidencia1.IsValidada) else "Otro"
                                newDelivery['ComentarioRechazo'] = TieneEvidencia1.ComentarioRechazo
                                arrFoliosEvidencias.append(newDelivery)
                        else:
                            newDelivery = {}
                            newDelivery['XD_IDPedido'] = Delivery.XD_IDPedido.XD_IDPedido
                            newDelivery['Delivery'] = Delivery.XD_IDPedido.Delivery.translate(
                                {ord(c): "" for c in "!@#$%^&*()[]{};:,./<>?\|`~-=_+"})
                            newDelivery['IDViaje'] = Delivery.XD_IDViaje.XD_IDViaje
                            newDelivery['TipoEvidencia'] = 'Pedido'
                            newDelivery[
                                'RutaArchivo'] = ""  # if (Delivery.IsEvidenciaPedidoxViaje or Delivery.IsEvidenciaFisicaPedidoxViaje) else ""
                            ...
                            newDelivery['ComentarioRechazo'] = ""
                            arrFoliosEvidencias.append(newDelivery)
            for Maniobras in XD_AccesoriosxViajes.objects.filter(XD_IDViaje=XDFolio.XD_IDViaje, Descripcion__in=(
            'Maniobras de descarga', 'Maniobras de carga')):
                if Maniobras:
                    # if len(GetManiobras) == 0:
                    if not XD_EvidenciasxViaje.objects.filter(IDXD_Viaje=Maniobras.XD_IDViaje,
                                                              Titulo=Maniobras.Descripcion,
                                                              Tipo="MESA CONTROL").exists():
                        newDelivery = {}
                        newDelivery[
                            'XD_IDPedido'] = Maniobras.XD_IDAccesorioxViaje  # if(len(GetManiobras) == 0) else Maniobras.XD_IDAccesorioxViaje if(GetEachManiobra.IsEnviada and GetEachManiobra.IsRechazada) else GetEachManiobra.IDEvidenciaxViaje
                        newDelivery[
                            'Delivery'] = Maniobras.Descripcion  # if(GetEachManiobra.IsEnviada and GetEachManiobra.IsRechazada) else Maniobras.Descripcion if(len(GetManiobras) == 0) else GetEachManiobra.Titulo
                        newDelivery[
                            'IDViaje'] = Maniobras.XD_IDViaje  # if(GetEachManiobra.IsEnviada and GetEachManiobra.IsRechazada) else Maniobras.XD_IDViaje if(len(GetManiobras) == 0) else GetEachManiobra.IDXD_Viaje
                        newDelivery['TipoEvidencia'] = 'Maniobras'
                        newDelivery[
                            'RutaArchivo'] = ''  # if(GetEachManiobra.IsEnviada and GetEachManiobra.IsRechazada) else GetEachManiobra.RutaArchivo if (GetEachManiobra.IsEnviada and not GetEachManiobra.IsRechazada and not GetEachManiobra.IsValidada) else GetEachManiobra.RutaArchivo
                        newDelivery[
                            'Status'] = 'Pendiente'  # if(GetEachManiobra.IsEnviada and GetEachManiobra.IsRechazada) else 'Aprobada' if(GetEachManiobra.IsEnviada and GetEachManiobra.IsValidada) else 'Pendiente' if len(GetManiobras) == 0 else "Enviada" if (GetEachManiobra.IsEnviada and not GetEachManiobra.IsRechazada and not GetEachManiobra.IsValidada) else "Otro"
                        newDelivery['ComentarioRechazo'] = ""
                        arrFoliosEvidencias.append(newDelivery)
                    else:
    ...
    try:
        for Evidencias in jParams['arrEvidencias']:
            with transaction.atomic(using='XD_ViajesDB'):
                TituloEvidencia = 'Maniobras de descarga' if (
                            Evidencias['Titulo'] == 'Maniobrasdedescarga') else 'Maniobras de carga' if (
                            Evidencias['Titulo'] == 'Maniobrasdecarga') else ""
                if Evidencias['Status'] == 'Rechazada':
                    if Evidencias['TipoEvidencia'] == 'BKG':
                        SaveEvidenciasxBKG = Bro_EvidenciasxViaje.objects.get(
                            IDBro_EvidenciaxViaje=Evidencias['IDPedido'])
                    ...
                    elif Evidencias['TipoEvidencia'] == 'Servicio':
                        SaveEvidenciaxServicio = XD_EvidenciasxViaje.objects.get(
                            IDEvidenciaxViaje=Evidencias["IDPedido"])
                        SaveEvidenciaxServicio.FechaCaptura = datetime.datetime.now()
                        SaveEvidenciaxServicio.NombreArchivo = Evidencias['NombreArchivo']
                        SaveEvidenciaxServicio.RutaArchivo = Evidencias['Evidencia']
                        SaveEvidenciaxServicio.IsRechazada = False
                        SaveEvidenciaxServicio.IsEnviada = True
                        SaveEvidenciaxServicio.IsRemplazada = True
                        SaveEvidenciaxServicio.IDUsuarioAlta = AdmonUsuarios.objects.get(
                            idusuario=request.user.idusuario)
                        SaveEvidenciaxServicio.save()
                        return HttpResponse(status=200)
                    else:
                        SaveEvidenciaxPedido = XD_EvidenciasxPedido.objects.get(IDXD_Pedido=Evidencias['IDPedido'],
                                                                                XD_IDViaje=Evidencias['IDViaje'],
                                                                                Titulo='EVIDENCIA1') if (
                                    Evidencias['TipoEvidencia'] == 'Pedido') else XD_EvidenciasxPedido.objects.get(
                            IDXD_Pedido=Evidencias['IDPedido'], XD_IDViaje=Evidencias['IDViaje'],
                            Titulo=Evidencias['TipoEvidencia'].upper()) if (
                                    Evidencias['TipoEvidencia'] == 'Bitacora') else XD_EvidenciasxViaje.objects.get(
                            IDEvidenciaxViaje=Evidencias['IDPedido'], IDXD_Viaje=Evidencias['IDViaje']) if (
                                    Evidencias['TipoEvidencia'] == 'Maniobras' or Evidencias[
                                'TipoEvidencia'] == 'Custodia') else ""
                    ...
                    if Evidencias['TipoEvidencia'] == 'BKG':
                        ...
                        SaveEvidenciaxServicio = XD_EvidenciasxViaje()
                        SaveEvidenciaxServicio.IDServicio = ServiciosXD.objects.get(IDServicio=Evidencias['IDViaje'])
                        SaveEvidenciaxServicio.IDUsuarioAlta = AdmonUsuarios.objects.get(
                            idusuario=request.user.idusuario)
                        SaveEvidenciaxServicio.FechaCaptura = datetime.datetime.now()
                        SaveEvidenciaxServicio.Titulo = "Servicios"
                        SaveEvidenciaxServicio.Tipo = "EVIDENCIA SERVICIOS"
                        SaveEvidenciaxServicio.NombreArchivo = Evidencias['NombreArchivo']
                        SaveEvidenciaxServicio.RutaArchivo = Evidencias['Evidencia']
                        SaveEvidenciaxServicio.Observaciones = ''
                        SaveEvidenciaxServicio.IsEnviada = True
                        SaveEvidenciaxServicio.save()
                        return HttpResponse(status=200)
                        ...
                        SaveEvidenciaxPedido = XD_EvidenciasxPedido() if (
                                    Evidencias['TipoEvidencia'] == 'Pedido' or Evidencias[
                                'TipoEvidencia'] == 'Bitacora') else XD_EvidenciasxViaje() if (
                                    Evidencias['TipoEvidencia'] == 'Maniobras' or Evidencias[
                                'TipoEvidencia'] == 'Custodia') else ""
                        SaveEvidenciaxPedido.IDXD_Pedido = Evidencias['IDPedido'] if (
                                    Evidencias['TipoEvidencia'] == 'Pedido' or Evidencias[
                                'TipoEvidencia'] == 'Bitacora') else None
                        ...
                        SaveEvidenciaxPedido.IDUsuarioAlta = AdmonUsuarios.objects.get(idusuario=request.user.idusuario)
                        SaveEvidenciaxPedido.FechaCaptura = datetime.datetime.now()
                        SaveEvidenciaxPedido.Titulo = 'EVIDENCIA1' if (
                                    Evidencias['TipoEvidencia'] == 'Pedido') else TituloEvidencia if (
                                    Evidencias['TipoEvidencia'] == 'Maniobras') else TituloCustodia if (
                                    Evidencias['TipoEvidencia'] == 'Custodia') else "BITACORA" if Evidencias[
                                                                                                      'TipoEvidencia'] == 'Bitacora' else ""
                        SaveEvidenciaxPedido.Tipo = 'EVIDENCIA' if (
                                    Evidencias['TipoEvidencia'] == 'Pedido' or Evidencias[
                                'TipoEvidencia'] == 'Bitacora') else 'MESA CONTROL' if (
                                    Evidencias['TipoEvidencia'] == 'Maniobras') else "EVCUSTODIAF" if (
                                    Evidencias['TipoEvidencia'] == 'Custodia') else ""
                        SaveEvidenciaxPedido.NombreArchivo = Evidencias['NombreArchivo']
                        SaveEvidenciaxPedido.RutaArchivo = Evidencias['Evidencia']
...
def GetEvidenciasMesaControl(request):
    ...
    try:
        if "FTL" in Folio:
            GetEvidenciasValidarBGK = Bro_EvidenciasxViaje.objects.filter(IDBro_Viaje=IDViaje, IsEnviada=1,
                                                                          IsRechazada=0, IsValidada=0)
        ...
        elif "XDDMNB" in Folio:
            GetEvidenciasServicios = XD_EvidenciasxViaje.objects.get(IDServicio=IDViaje, IsEnviada=1, IsRechazada=0,
                                                                     IsValidada=0)
            AddEvidencia = {}
            AddEvidencia['IDEvidencia'] = GetEvidenciasServicios.IDEvidenciaxViaje
            AddEvidencia['URLEvidencia'] = GetEvidenciasServicios.RutaArchivo
            AddEvidencia['Delivery'] = GetEvidenciasServicios.Titulo
            AddEvidencia['TipoEvidencia'] = 'Servicio'
            AddEvidencia['IDViaje'] = GetEvidenciasServicios.IDServicio.IDServicio
            ListEvidencias.append(AddEvidencia)
            ...
            if len(GetIDPedidos) >= 1:
                ...
                if XD_EvidenciasxPedido.objects.filter(XD_IDViaje=IDViaje).count() == TotalEvidencias:
                    for GetPedidos in GetIDPedidos:
                        ...
                        GetEvidenciaxPedido = XD_EvidenciasxPedido.objects.filter(
                            IDXD_Pedido=GetPedidos.XD_IDPedido.XD_IDPedido, XD_IDViaje=IDViaje)
                        for eachEvidenciaxPedido in GetEvidenciaxPedido:
                            if eachEvidenciaxPedido.IsEnviada and not eachEvidenciaxPedido.IsValidada and not eachEvidenciaxPedido.IsRechazada:
                                ...
                                AddEvidencia['Delivery'] = GetDelivery.Delivery.translate({ord(c): "" for c in
                                                                                           "!@#$%^&*()[]{};:,./<>?\|`~-=_+"}) if eachEvidenciaxPedido.Titulo != "BITACORA" else GetObservacionesByPedidoT1(
                                    GetDelivery.Observaciones, GetPedidos.TipoTransporte)
                                AddEvidencia[
                                    'TipoEvidencia'] = 'Pedido' if eachEvidenciaxPedido.Titulo != "BITACORA" else 'Bitacora'
                                AddEvidencia['IDViaje'] = eachEvidenciaxPedido.XD_IDViaje
                                ListEvidencias.append(AddEvidencia)
                else:
                    return HttpResponse(status=500)
            GetEvidenciasxViaje = XD_EvidenciasxViaje.objects.filter(
                Q(IDXD_Viaje=IDViaje, Titulo__in=('Maniobras de descarga', 'Maniobras de carga'),
                  Tipo='MESA CONTROL') | Q(IDXD_Viaje=IDViaje, Tipo='EVCUSTODIAF'))
            ...
            with transaction.atomic(using='bkg_viajesDB'):
                with transaction.atomic(using='users'):
                    if jParams['TipoEvidencia'] == 'Pedido' or jParams['TipoEvidencia'] == 'Bitacora':
                        SaveEvidenciaxPedido = XD_EvidenciasxPedido.objects.get(
                            IDEvidenciaxPedido=jParams['IDSaveEvidencia'])
                        ...
                        SaveEvidenciaxPedido.save()
                        SaveBanderaPedidoxviaje = XD_PedidosxViajes.objects.get(
                            XD_IDPedido=SaveEvidenciaxPedido.IDXD_Pedido, XD_IDViaje=SaveEvidenciaxPedido.XD_IDViaje)
                        ...
                        IsXpress = XD_Viajes.objects.get(
                            XD_IDViaje=SaveEvidenciaxPedido.XD_IDViaje)  # IsViajeXpress(SaveEvidenciaxPedido.IDXD_Pedido, SaveEvidenciaxPedido.XD_IDViaje)
                        if IsXpress.TipoViaje == "XPRESS":
                            GetAllIDEvidenciasPedidos = XD_PedidosxViajes.objects.filter(
                                XD_IDPedido=SaveEvidenciaxPedido.IDXD_Pedido)
                            for SaveEachPedidoPE in GetAllIDEvidenciasPedidos:
                                GetIDPendientesEnviar = RelacionConceptoxProyecto.objects.filter(
                                    IDConcepto=SaveEachPedidoPE.XD_IDPedido.XD_IDPedido).values('IDPendienteEnviar')
                                for VerificarTipoConcepto in GetIDPendientesEnviar:
                                    SaveBanderaPendientesEnviar = PendientesEnviar.objects.get(
                                        IDPendienteEnviar=VerificarTipoConcepto['IDPendienteEnviar'])
                        ...
                        if SaveXD_EDigital:
                            SaveXD_EDigital = XD_Viajes.objects.get(
                                XD_IDViaje=SaveBanderaPedidoxviaje.XD_IDViaje.XD_IDViaje)
                            SaveXD_EDigital.IsEvidenciaPedidos = True
                            SaveXD_EDigital.FechaEvidenciaDigital = datetime.datetime.now()
                            SaveXD_EDigital.save()
                            VerificarEvidenciaT1yT2 = GetAllEvidecesDigitalsT1yT2(
                                SaveBanderaPedidoxviaje.XD_IDViaje.XD_IDViaje)
                            if VerificarEvidenciaT1yT2:
                                VerificarValidacionesEvidencias = VerificarSaveViajePE(
                                    SaveBanderaPedidoxviaje.XD_IDPedido.XD_IDPedido)
                                for EachViaje in VerificarValidacionesEvidencias:
                                    GetIDPendientesEnviar = RelacionConceptoxProyecto.objects.filter(
                                        IDConcepto=EachViaje).values('IDPendienteEnviar')
                                    for VerificarTipoConcepto in GetIDPendientesEnviar:
                                        SaveBanderaPendientesEnviar = PendientesEnviar.objects.get(
                                            IDPendienteEnviar=VerificarTipoConcepto['IDPendienteEnviar'])
                    ...
                    if jParams['TipoEvidencia'] == 'Maniobras' or jParams['TipoEvidencia'] == 'Custodia':
                        SaveEvidenciaxManiobra = XD_EvidenciasxViaje.objects.get(
                            IDEvidenciaxViaje=jParams['IDSaveEvidencia'])
                        ...
                        if SaveXD_EDigital:
                            ...
                            if jParams['TipoEvidencia'] == 'Custodia':
                                GetIDPendientesEnviar1 = RelacionConceptoxProyecto.objects.filter(
                                    IDConcepto=SaveXD_EDigital1.XD_IDViaje)
                                for TC in GetIDPendientesEnviar1:
                                    SaveBanderaPendientesEnviar = PendientesEnviar.objects.get(
                                        IDPendienteEnviar=TC.IDPendienteEnviar.IDPendienteEnviar)
                                    if SaveBanderaPendientesEnviar.TipoConcepto == 'VIAJE':
                                        SaveBanderaPendientesEnviar.IsEvidenciaDigital = True
                                        SaveBanderaPendientesEnviar.save()
                                ...
                                for TC in GetIDPendientesEnviar1:
                                    ...
                                    if SaveBanderaPendientesEnviar.TipoConcepto == 'VIAJE':
                                        SaveBanderaPendientesEnviar.IsEvidenciaDigital = True
                                        SaveBanderaPendientesEnviar.save()
                    if jParams['TipoEvidencia'] == 'Servicio':
                        SaveAprobarEVServicio = XD_EvidenciasxViaje.objects.get(
                            IDEvidenciaxViaje=jParams['IDSaveEvidencia'])
                        SaveAprobarEVServicio.IsValidada = True
                        SaveAprobarEVServicio.Observaciones = jParams['Comentarios']
                        SaveAprobarEVServicio.FechaValidacion = datetime.datetime.now()
                        SaveAprobarEVServicio.IDUsuarioEvDigital = request.user.idusuario
                        SaveAprobarEVServicio.save()
                        if AllEvServiciosTrue("", SaveAprobarEVServicio.IDServicio.IDServicio):
                            GetServicio = ServiciosXD.objects.get(
                                IDServicio=SaveAprobarEVServicio.IDServicio.IDServicio)
                            GetServicio.IsEvidenciaServicio = True
                            GetServicio.FechaEvidenciaDigital = SaveAprobarEVServicio.FechaValidacion
                            GetServicio.save()
                            for eachConcept in RelacionConceptoxProyecto.objects.filter(
                                    IDConcepto=SaveAprobarEVServicio.IDServicio.IDServicio):
                                GetIDServicioPE = PendientesEnviar.objects.get(
                                    IDPendienteEnviar=eachConcept.IDPendienteEnviar.IDPendienteEnviar)
                                if GetIDServicioPE.TipoConcepto == 'SERVICIO':
                                    GetIDServicioPE.IsEvidenciaDigital = True
                                    GetIDServicioPE.save()
                    if jParams['TipoEvidencia'] == 'BKG':
                        SaveAprobarEvidencia = Bro_EvidenciasxViaje.objects.get(
                            IDBro_EvidenciaxViaje=jParams['IDSaveEvidencia'])
                        ...
                        SaveEvidenciaDigitalViaje = EvidenciaDigitalCompletaBKG("",
                                                                                SaveAprobarEvidencia.IDBro_Viaje.IDBro_Viaje)
                        if SaveEvidenciaDigitalViaje:
                            GetEvDViaje = Bro_Viajes.objects.get(
                                IDBro_Viaje=SaveAprobarEvidencia.IDBro_Viaje.IDBro_Viaje)
                            GetEvDViaje.IsEvidenciasDigitales = True
                            GetEvDViaje.FechaRecEviDigitales = datetime.datetime.now()
                            GetEvDViaje.save()
                            GetIDPendientesEnviar = RelacionConceptoxProyecto.objects.get(
                                IDConcepto=SaveAprobarEvidencia.IDBro_Viaje.IDBro_Viaje)
                            GetEvDAdmon = PendientesEnviar.objects.get(
                                IDPendienteEnviar=str(GetIDPendientesEnviar.IDPendienteEnviar))
...
def RechazarEvidencias(request):
    jParams = json.loads(request.body.decode('utf-8'))
    if jParams['TipoEvidencia'] == 'Servicio':
        RechazarEvidenciaxPedido = XD_EvidenciasxViaje.objects.get(IDEvidenciaxViaje=jParams['IDRechazarEvidencia'])
    ...
    elif jParams['TipoEvidencia'] == 'Pedido' or jParams['TipoEvidencia'] == 'Bitacora':
...
def ValidarEvidenciaXD_Viajea(IDViaje):
    ...
    AllEvidencesManiobrasTrue = XD_EvidenciasxViaje.objects.filter(
    ...
    if len(AllEvidencesManiobrasTrue) >= 1:
        for b in AllEvidencesManiobrasTrue:
            listEvidenciasBool.append(b.IsEvidenciaFisicaAprobada) if (
                        b.Tipo == 'EVCUSTODIAF') else listEvidenciasBool.append(b.IsValidada)
...
def GetEvidenciaFisica(request):
    ...
    if "FTL" in Folio:
        ...
        IsValidada = XD_EvidenciasxViaje.objects.filter(IDServicio=IDViaje, IsValidada=1, IsRechazada=0, IsEnviada=1,
                                                        IsEvidenciaFisicaAprobada=0).exists()
        if IsValidada:
            EvFisica = XD_EvidenciasxViaje.objects.get(IDServicio=IDViaje, IsValidada=1, IsRechazada=0, IsEnviada=1,
                                                       IsEvidenciaFisicaAprobada=0)
            newEvidenciasFisicas = {}
            newEvidenciasFisicas["XD_IDPedido"] = EvFisica.IDEvidenciaxViaje
            newEvidenciasFisicas["XD_IDViaje"] = EvFisica.IDServicio.IDServicio
            newEvidenciasFisicas["Delivery"] = EvFisica.IDServicio.Folio
            EvidenciaFisica.append(newEvidenciasFisicas)
            return JsonResponse({'EvidenciaFisica': EvidenciaFisica})
        else:
            return HttpResponse(status=500)
    else:
        for TieneEvidenciaValidada in XD_EvidenciasxViaje.objects.filter(
                Q(IDXD_Viaje=IDViaje, Titulo__in=('Maniobras de descarga', 'Maniobras de carga'),
                  Tipo="MESA CONTROL") | Q(IDXD_Viaje=IDViaje, Tipo='EVCUSTODIAF')).values('IsValidada'):
            if len(TieneEvidenciaValidada) >= 1:
                TieneEvidenciasEnFalse.append(TieneEvidenciaValidada['IsValidada'])
        for TieneEvidenciaDigital in XD_PedidosxViajes.objects.filter(XD_IDViaje=IDViaje).values(
                'IsEvidenciaPedidoxViaje'):
        ...
        if False in TieneEvidenciasEnFalse:
            ...
            for delibery in XD_EvidenciasxViaje.objects.filter(IDXD_Viaje=IDViaje, IsEvidenciaFisicaAprobada=0) if (
                    VerificarTipoEvidencia.TipoViaje == 'CUSTODIA') else XD_PedidosxViajes.objects.filter(
                    XD_IDViaje=IDViaje):
                if delibery.IsEnviada if (
                        VerificarTipoEvidencia.TipoViaje == 'CUSTODIA') else not delibery.IsEvidenciaFisicaPedidoxViaje:
                    newEvidenciasFisicas = {}
                    newEvidenciasFisicas["XD_IDPedido"] = delibery.IDEvidenciaxViaje if (
                                VerificarTipoEvidencia.TipoViaje == 'CUSTODIA') else delibery.XD_IDPedido.XD_IDPedido
                    newEvidenciasFisicas["XD_IDViaje"] = delibery.IDXD_Viaje if (
                                VerificarTipoEvidencia.TipoViaje == 'CUSTODIA') else delibery.XD_IDViaje.XD_IDViaje
                    newEvidenciasFisicas["Delivery"] = delibery.Titulo if (
                                VerificarTipoEvidencia.TipoViaje == 'CUSTODIA') else delibery.XD_IDPedido.Delivery
                    EvidenciaFisica.append(newEvidenciasFisicas)
    return JsonResponse({'EvidenciaFisica': EvidenciaFisica})
...
def SaveEvidenciaFisica(request):
    ...
    try:
            ...
            with transaction.atomic(using='bkg_viajesDB'):
                with transaction.atomic(using='users'):
                    if "BKG" in str(jParams["TipoEvidencia"]):
                        ...
                        if allEvFisicaTrue:
                            SaveEvFisicaViajes = Bro_Viajes.objects.get(
                                IDBro_Viaje=SaveEvFisica.IDBro_Viaje.IDBro_Viaje)
                            ...
                            GetIDPendientesEnviar = RelacionConceptoxProyecto.objects.get(
                                IDConcepto=SaveEvFisica.IDBro_Viaje.IDBro_Viaje)
                            SaveEvFisicaAdmon = PendientesEnviar.objects.get(
                                IDPendienteEnviar=str(GetIDPendientesEnviar.IDPendienteEnviar))
                        ...
                        SaveEvFisicaServicio.IDUsuarioEvFisica = request.user.idusuario
                        SaveEvFisicaServicio.save()
                        if AllEvFisicaServicios("", SaveEvFisicaServicio.IDServicio.IDServicio):
                            GetServicio = ServiciosXD.objects.get(IDServicio=SaveEvFisicaServicio.IDServicio.IDServicio)
                            GetServicio.IsEvidenciaFisica = True
                            GetServicio.FechaEvidenciaFisica = SaveEvFisicaServicio.FechaEvidenciaFisicaxPedidoxViaje
                            GetServicio.save()
                            for eachConcept in RelacionConceptoxProyecto.objects.filter(
                                    IDConcepto=SaveEvFisicaServicio.IDServicio.IDServicio):
                                GetIDServicioPE = PendientesEnviar.objects.get(
                                    IDPendienteEnviar=eachConcept.IDPendienteEnviar.IDPendienteEnviar)
                                if GetIDServicioPE.TipoConcepto == 'SERVICIO':
                                    GetIDServicioPE.IsEvidenciaFisica = True
                                    GetIDServicioPE.save()
                    elif "XDDMNB" not in str(jParams["TipoEvidencia"]) and "BKG" not in str(jParams["TipoEvidencia"]):
                        SaveEvidenciaPedidosxViaje = XD_EvidenciasxViaje.objects.get(
                            IDEvidenciaxViaje=jParams['IDPedido']) if (str(jParams["TipoEvidencia"]) == 'FOLIO' or str(
                            jParams["TipoEvidencia"]) == 'CORREO') else XD_PedidosxViajes.objects.get(
                            XD_IDPedido=jParams['IDPedido'], XD_IDViaje=jParams['IDViaje'])
                        ...
                        if str(jParams["TipoEvidencia"]) != 'FOLIO' or str(jParams["TipoEvidencia"]) != 'CORREO':
                            ...
                            if IsExpress.TipoViaje == 'XPRESS':
                                GetIDPE = RelacionConceptoxProyecto.objects.filter(
                                    IDConcepto=jParams['IDPedido']).values("IDPendienteEnviar")
                                for Conceptos in GetIDPE:
                                    SaveEVFisPE = PendientesEnviar.objects.get(
                                        IDPendienteEnviar=Conceptos["IDPendienteEnviar"])
                        ...
                        if SaveXD_Viajes:
                            ...
                            if len(PedidoStatus) != 0:
                                for GetPedidoStatus in PedidoStatus:
                                    SavePedidoStatus = XD_PedidosxViajes.objects.get(
                                        XD_IDPedido=GetPedidoStatus.XD_IDPedido, XD_IDViaje=GetPedidoStatus.XD_IDViaje)
                                    SavePedidoStatus.StatusPedido = 'COMPLETO'
                                    SavePedidoStatus.save()
                            GetIDPendientesEnviar = RelacionConceptoxProyecto.objects.filter(
                                IDConcepto=jParams['IDViaje']).values('IDPendienteEnviar')
        ...
        api = APIHojaLiberacionServicios(SaveEvFisicaServicio.IDServicio.IDServicio) if "XDDMNB" in str(
            jParams["TipoEvidencia"]) else descarga(jParams['IDViaje'], str(jParams["TipoEvidencia"]))
        return HttpResponse(status=200)
    except Exception as e:
        transaction.rollback(using='XD_ViajesDB')
        transaction.rollback(using='bkg_viajesDB')
...
def EvidenciaDigitalCompleta(request, viaje=""):
    ...
    if ClienteFiscal.IDClienteFiscal != settings.ID_CLIENTE_FISCAL:
...
def TotalEvidenciasA(IDViaje):
    lista = list()
    if XD_AccesoriosxViajes.objects.filter(XD_IDViaje=IDViaje, Descripcion__in=(
            'Maniobras de descarga', 'Maniobras de carga')).count() == XD_EvidenciasxViaje.objects.filter(
        IDXD_Viaje=IDViaje, Titulo__in=(
    ...
    try:
        ...
        if not Bro_EvidenciasxViaje.objects.filter(IDBro_Viaje=GetIDViaje.IDBro_Viaje).exists():
            ...
            for Evidencias in GetEvidencias:
                ...
                AddNewDataToJdon['Status'] = 'Enviada' if (
                            Evidencias.IsEnviada and not Evidencias.IsRechazada and not Evidencias.IsValidada) else 'Rechazada' if (
                            Evidencias.IsEnviada and Evidencias.IsRechazada) else 'Aprobada' if (
                            Evidencias.IsEnviada and Evidencias.IsValidada) else 'Otro'
...
def DescargarHojaLiberacion(request):
    ...
    if Proyecto == 'BKG':
        try:
            GetRutaHojaLiberacion = Bro_Viajes.objects.get(IDBro_Viaje=IDViaje, IsEvidenciasDigitales=1,
                                                           IsEvidenciasFisicas=1)
            ...
            return HttpResponse(status=500)
    elif Proyecto == 'Servicio':
        try:
            GetRutaHojaLiberacion = ServiciosXD.objects.get(IDServicio=IDViaje, IsEvidenciaServicio=1,
                                                            IsEvidenciaFisica=1)
            if not GetRutaHojaLiberacion.IsDescargaHojaLiberacion or GetRutaHojaLiberacion.IsDescargaHojaLiberacion is None:
...
def FilterBy(request):
    ...
    if "Year" in request.GET:
        ...
        if ("BKG" in Proyectos and "XD" in Proyectos) or Proyectos == []:
            EvViajesBKG = Bro_Viajes.objects.filter(FechaDescarga__month__in=arrMonth, FechaDescarga__year=Year)
            EvViajesXD = XD_Viajes.objects.filter(FechaDespacho__month__in=arrMonth, FechaDespacho__year=Year)
            EvServicios = ServiciosXD.objects.filter(FechaAlta__month__in=arrMonth, FechaAlta__year=Year)
            Evidencias = chain(EvViajesBKG, EvViajesXD, EvServicios)
            ...
            EvViajesXD = XD_Viajes.objects.filter(FechaDespacho__month__in=arrMonth, FechaDespacho__year=Year)
            EvServicios = ServiciosXD.objects.filter(FechaAlta__month__in=arrMonth, FechaAlta__year=Year)
            Evidencias = chain(EvViajesXD, EvServicios)
    else:
        if "BKG" in Proyectos and "XD" in Proyectos:
            EvViajesBKG = Bro_Viajes.objects.filter(
                FechaDescarga__range=[datetime.datetime.strptime(request.GET["FechaFacturaDesde"], '%m/%d/%Y'),
                                      datetime.datetime.strptime(request.GET["FechaFacturaHasta"], '%m/%d/%Y')])
            EvViajesXD = XD_Viajes.objects.filter(
                FechaDespacho__range=[datetime.datetime.strptime(request.GET["FechaFacturaDesde"], '%m/%d/%Y'),
                                      datetime.datetime.strptime(request.GET["FechaFacturaHasta"], '%m/%d/%Y')])
            EvServicios = ServiciosXD.objects.filter(
                FechaAlta__range=[datetime.datetime.strptime(request.GET["FechaFacturaDesde"], '%m/%d/%Y'),
                                  datetime.datetime.strptime(request.GET["FechaFacturaHasta"], '%m/%d/%Y')])
            Evidencias = chain(EvViajesBKG, EvViajesXD, EvServicios)
        elif "BKG" in Proyectos and "XD" not in Proyectos:
            Evidencias = Bro_Viajes.objects.filter(
                FechaDescarga__range=[datetime.datetime.strptime(request.GET["FechaFacturaDesde"], '%m/%d/%Y'),
                                      datetime.datetime.strptime(request.GET["FechaFacturaHasta"], '%m/%d/%Y')])
        elif "XD" in Proyectos and "BKG" not in Proyectos:
            EvXD = XD_Viajes.objects.filter(
                FechaDespacho__range=[datetime.datetime.strptime(request.GET["FechaFacturaDesde"], '%m/%d/%Y'),
                                      datetime.datetime.strptime(request.GET["FechaFacturaHasta"], '%m/%d/%Y')])
            EvServicios = ServiciosXD.objects.filter(
                FechaAlta__range=[datetime.datetime.strptime(request.GET["FechaFacturaDesde"], '%m/%d/%Y'),
                                  datetime.datetime.strptime(request.GET["FechaFacturaHasta"], '%m/%d/%Y')])
            Evidencias = chain(EvXD, EvServicios)
    ...
    if Proveedores:
        if ("BKG" in Proveedores and "XD" in Proveedores) or Proveedores == []:
            ProvBKG = Bro_Viajes.objects.filter(IDTransportista__in=Proveedores)
            ProvXD = XD_Viajes.objects.filter(IDTransportista__in=Proveedores)
            ProvServ = ServiciosXD.objects.filter(IDTransportista__in=Proveedores)
            Evidencias = chain(ProvBKG, ProvXD, ProvServ)
...
def descarga(IDViaje, Proyecto):
    sendAPI = CheckAllEvidenciaFisicaPedidoTrue(IDViaje) if str(Proyecto[:3]) != "BKG" else CheckAllEvidenciaFisicaTrue(
        IDViaje)

    if sendAPI:
        Project = "BKG" if str(Proyecto[:3]) == "BKG" else "XD"
        jsonParams = {'IDConcepto': IDViaje, 'Proyecto': Project}
        respose = requests.post(settings.API_ADMON + "api/Usuarios/SaveFolioHojaLiberacion",
...
def APIHojaLiberacionServicios(IDServicio):
    SendApi = AllEvFisicaServicios("", IDServicio)
    if SendApi:
        jsonParams = {'IDConcepto': IDServicio, 'Proyecto': 'SERVICIO'}
        respose = requests.post(settings.API_ADMON + "api/Usuarios/SaveFolioHojaLiberacion",
                                headers={'content-type': 'application/json'}, json=jsonParams)
        return respose.status_code
    else:
...
def fechaevidencias(request):
    viajes = XD_Viajes.objects.filter(Status='FINALIZADO', IsEvidenciaFisica=1, FechaDespacho__gte='2020-07-01',
                                      FechaEvidenciaFisica=None)
    for eachviaje in viajes:
        ...
        if eachviaje.TipoViaje != "CUSTODIA":
            ...
            if fechapedidoEv.FechaValidacion is not None:
                saveFecha.FechaEvidenciaFisica = fechapedidoEv.FechaValidacion + datetime.timedelta(
            ...
            fechacustodia = XD_EvidenciasxViaje.objects.filter(IDXD_Viaje=eachviaje.XD_IDViaje,
                                                               Tipo='EVCUSTODIAF').last()
            saveFecha = XD_Viajes.objects.get(XD_IDViaje=eachviaje.XD_IDViaje)
            ...
            if fechacustodia.FechaEvidenciaFisicaxPedidoxViaje is None:
                ...
                for i in eachEv:
                    ...
                    if fechacustodia.FechaValidacion is not None:
                        saveEv.FechaEvidenciaFisicaxPedidoxViaje = fechacustodia.FechaValidacion + datetime.timedelta(
                            days=2)
                    else:
    ...
    try:
        if "FTL" in Folio:
            ...
            for evidencias in getEvidenciaxPedido:
                jsonevidencias = {}
                TituloPedido = XD_PedidosxViajes.objects.get(XD_IDPedido=evidencias.IDXD_Pedido,
                                                             XD_IDViaje=evidencias.XD_IDViaje)
                jsonevidencias[
                    "Titulo"] = TituloPedido.XD_IDPedido.Delivery if evidencias.Titulo != "BITACORA" else GetObservacionesByPedidoT1(
                    TituloPedido.XD_IDPedido.Observaciones,
                    "T1") if evidencias.Titulo == 'BITACORA' and TituloPedido.TipoTransporte == 'T1' else GetObservacionesByPedidoT1(
                    TituloPedido.XD_IDPedido.Observaciones,
                    "T2") if evidencias.Titulo == 'BITACORA' and TituloPedido.TipoTransporte == 'T2' else None
                jsonevidencias["RutaArchivo"] = evidencias.RutaArchivo
                ListEvidencias.append(jsonevidencias)
            getEvidenciaxCustodia = XD_EvidenciasxViaje.objects.filter(IDXD_Viaje=IDViaje, Tipo="EVCUSTODIAF",
                                                                       Titulo__in=('CORREO', 'FOLIO'))
            for evidenciasCustodias in getEvidenciaxCustodia:
                ...
                ListEvidencias.append(jsonevidencias)
            gerEvidenciasxManiobras = XD_EvidenciasxViaje.objects.filter(IDXD_Viaje=IDViaje, Tipo__in=(
            "MESA CONTROL", "EVIDENCIA ACCESORIOS"), Titulo__in=('Maniobras de carga', 'Maniobras de descarga'))
...
def GetObservacionesByPedidos(IDViaje):
    ...
    for pedido in GetDelivery:
        if pedido.XD_IDPedido.IDClienteFiscal == settings.ID_CLIENTE_FISCAL:
...
def GetEvidenciasMC(request):
    try:
        ...
        if not XD_EvidenciasxViaje.objects.filter(IDXD_Viaje=IDViaje, Tipo='EVCUSTODIAF', Titulo='CORREO',
                                                  IsEnviada=1).exists() or XD_EvidenciasxViaje.objects.filter(
                IDXD_Viaje=IDViaje, Tipo='EVCUSTODIAF', Titulo='CORREO', IsEnviada=1, IsRechazada=1).exists():
            XDFolio = XD_Viajes.objects.exclude(Status='CANCELADO').get(XD_IDViaje=IDViaje)
            data = XD_EvidenciasxViaje.objects.get(IDXD_Viaje=IDViaje, Tipo='EVCUSTODIAF', Titulo='CORREO', IsEnviada=1,
                                                   IsRechazada=1) if XD_EvidenciasxViaje.objects.filter(
                IDXD_Viaje=IDViaje, Tipo='EVCUSTODIAF', Titulo='CORREO', IsEnviada=1, IsRechazada=1).exists() else ""
            newDelivery = {}
            newDelivery['XD_IDPedido'] = data.IDEvidenciaxViaje if XD_EvidenciasxViaje.objects.filter(
                IDXD_Viaje=IDViaje, Tipo='EVCUSTODIAF', Titulo='CORREO', IsEnviada=1,
                IsRechazada=1).exists() else XDFolio.XD_IDViaje
            newDelivery['Delivery'] = XDFolio.Folio + "-" + "CORREO"
            newDelivery['IDViaje'] = XDFolio.XD_IDViaje
            newDelivery['TipoEvidencia'] = 'Custodia'
            newDelivery['RutaArchivo'] = ''
            newDelivery['Status'] = 'Rechazada' if XD_EvidenciasxViaje.objects.filter(IDXD_Viaje=IDViaje,
                                                                                      Tipo='EVCUSTODIAF',
                                                                                      Titulo='CORREO', IsEnviada=1,
                                                                                      IsRechazada=1).exists() else "Pendiente"
            newDelivery['ComentarioRechazo'] = ""
...
def DownloadHojaLiberacion(request, **kwargs):
    ...
    if Folio[:3] == "FTL" or Folio[:3] == "FTI":
        ...
        if GetDatosViaje.IsEvidenciasDigitales and GetDatosViaje.IsEvidenciasFisicas and GetDatosViaje.IsDescargaHojaLiberacion and not Ext_Viajes_MesaControl.objects.filter(
                IDBro_Viaje=IDViaje,
                IsDescargaHojaLiberacionMC=True).exists() if request.user.roles == "MesaControl" else not Ext_Viajes_MesaControl.objects.filter(
                IDBro_Viaje=IDViaje, IsDescargaHojaLiberacionCXP=True).exists():
        ...
        if GetDatosViaje.IsEvidenciaPedidos and GetDatosViaje.IsEvidenciaFisica and GetDatosViaje.IsDescargaHojaLiberacion and not Ext_Viajes_MesaControlXD.objects.filter(
                XD_IDViaje=IDViaje,
                IsDescargaHojaLiberacionMC=True).exists() if request.user.roles == "MesaControl" else not Ext_Viajes_MesaControlXD.objects.filter(
                XD_IDViaje=IDViaje, IsDescargaHojaLiberacionCXP=True).exists():
...
def SaveExtViajes(request, IDViaje, Proyecto):
    try:
        with transaction.atomic(using='bkg_viajesDB') if Proyecto == "BKG" else transaction.atomic(using='XD_ViajesDB'):
            if Proyecto == "BKG":
                SaveExtViajeMC = Ext_Viajes_MesaControl() if not Ext_Viajes_MesaControl.objects.filter(
                    IDBro_Viaje=IDViaje).exists() else Ext_Viajes_MesaControl.objects.get(IDBro_Viaje=IDViaje)
                SaveExtViajeMC.IDBro_Viaje = Bro_Viajes.objects.get(IDBro_Viaje=IDViaje)
            else:
                SaveExtViajeMC = Ext_Viajes_MesaControlXD() if not Ext_Viajes_MesaControlXD.objects.filter(
                    XD_IDViaje=IDViaje).exists() else Ext_Viajes_MesaControlXD.objects.get(XD_IDViaje=IDViaje)
...
def CountTotalEvidencias(Pedidos):
    CountList = list()
    if Pedidos[0].XD_IDViaje.IDClienteFiscal == settings.ID_CLIENTE_FISCAL:
        for eachPedido in Pedidos:
            observaciones = {}
            observaciones["name"] = GetObservacionesByPedidoT1(eachPedido.XD_IDPedido.Observaciones,
                                                               eachPedido.TipoTransporte)
            "" if observaciones["name"] == '' or observaciones["name"] == None or observaciones[
                "name"] == [] else CountList.append(observaciones)
...
def AllEvServiciosTrue(request, IDServicio=""):
    try:
        ...
        if IDServicio != "":
            return all(AllEvidenciasServicios)
        else:
            return JsonResponse({'data': all(AllEvidenciasServicios)})
    except Exception as e:
        print(e)
        return False


def AllEvFisicaServicios(request, IDServicio=""):
    Servicio = IDServicio if IDServicio != "" else request.GET["IDViaje"]
    AllEvServiciosFisica = list()
    for eachEv in XD_EvidenciasxViaje.objects.filter(IDServicio=Servicio):
        AllEvServiciosFisica.append(eachEv.IsEvidenciaFisicaAprobada)
    if IDServicio != "":
        return all(AllEvServiciosFisica)
    else:
        return JsonResponse({'data': all(AllEvServiciosFisica)})
